...
public class ExceptionHandlerTests
{
    ...
    public async Task InvokeAsync_WhenHttpException_ReturnsCustomStatusAndMessage()
    {
        ...
        // HttpException j� � p�blica e recebe status, mensagem e erro
        var httpEx = new HttpException(404, "N�o encontrado", "N�o encontrado");
        ...
        Assert.Equal("N�o encontrado", json.RootElement.GetProperty("message").GetString());
        Assert.Equal("N�o encontrado", json.RootElement.GetProperty("error").GetString());
    ...
    public async Task InvokeAsync_WhenGenericException_ReturnsInternalServerError()
    {
        ...
        context.Response.Body = new System.IO.MemoryStream();
        var loggerMock = new Mock<ILogger<ExceptionHandler>>();
        var ex = new Exception("Erro gen�rico");
        ...
        // Parse do JSON e valida��o das propriedades
        var json = JsonDocument.Parse(response);
        Assert.Equal("Erro gen�rico", json.RootElement.GetProperty("message").GetString());
